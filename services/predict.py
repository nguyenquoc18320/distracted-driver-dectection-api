...
def add_distrac(category, path, userid) -> Distraction:
    if get_distraction_by_pathimage(path) is None:
        ...
        new_Distrac = Distraction(time=datetime.now(), 
                                category=category, 
                                image_path=path, \
...
def add_no_person(category, path, userid) -> Distraction:
    if get_distraction_by_pathimage(path) is None:
...
def get_distraction_by_pathimage(path) -> Distraction:
    session = Session()
    user_result = session.query(Distraction).filter(
        Distraction.image_path == path)
    session.close()
